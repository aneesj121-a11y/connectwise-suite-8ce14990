  ...
  return (
    <div className="h-[calc(100vh-3.5rem)] flex">
      <aside className="w-[340px] shrink-0 border-r border-border bg-card flex flex-col">
        <div className="px-4 py-3 border-b border-border">
          <div className="flex items-center justify-between">
            <h1 className="font-display text-base font-semibold">Help Desk</h1>
            <span className="text-xs text-muted-foreground">{TICKETS.filter((t) => t.unread).length} unread</span>
          </div>
          <div className="mt-2 flex gap-1 text-[11px] flex-wrap">
            {["All", "Chats", "Emails", "Calls", "Mine", "P1"].map((c, i) => (
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-0.5">
                {active.id} · {active.customer} · {active.channel} · Assigned to {active.assignee}
              </div>
            </div>
      ...
      <aside className="w-[280px] shrink-0 border-l border-border bg-card overflow-y-auto p-4 space-y-4 hidden xl:block">
        ...
        <div>
          <div className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground mb-2">Linked tickets</div>
          <ul className="space-y-1.5 text-xs">
            <li className="flex justify-between"><span className="text-muted-foreground">#4821</span><span>Billing escalation</span></li>
            <li className="flex justify-between"><span className="text-muted-foreground">#4756</span><span>SSO config</span></li>
            <li className="flex justify-between"><span className="text-muted-foreground">#4702</span><span>Onboarding call</span></li>
          </ul>
        </div>
        <div>
